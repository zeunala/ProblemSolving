...
- time.time()함수로 분석해보니 countNation함수 부분에서 시간이 너무 많이 쓰여서 함수 호출을 안하고 최대한 시간을 줄이는 방향으로 진행하였다.
* Pass/3rd/02:39:57(use PyPy3)
삼성역량테스트에서 채점하는 데 쓰는 pypy3으로는 정답처리가 되었으나 python3에서는 여전히 시간초과가 발생하였으므로 최적화가 더 필요할 것이다.
'''
...
while True: # 여기서 부터 한단계의 전체과정
    ...
    idx = 0
    totalIdx = [[] for _ in range(N*N)] # n번 연합에 해당하는 것들
    sumIdx = [0 for _ in range(N*N)] # n번 연합에 해당하는 것들 총합
    lenIdx = [0 for _ in range(N*N)] # n번 연합에 해당하는 것들의 개수
    for i in range(N): # 우선 연합을 결정 (BFS 이용)
        for j in range(N):
            ...
            while queue:
                (m, n) = queue.popleft()
                if(link[m][n]==idx): continue
                link[m][n] = idx
                totalIdx[idx].append((m, n))
                sumIdx[idx]+=A[m][n]
                lenIdx[idx]+=1
    ...
    for i in range(idx): # 그리고 연합별로 합치고 분배
        for (m, n) in totalIdx[i]: # 분배
            newA[m][n]=sumIdx[i]//lenIdx[i]
